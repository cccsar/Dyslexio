...
process :: String -> BE.UserState -> IO BE.UserState 
process inputLine ustate = checkLexErrors inputLine validateAction ustate--putStrLn $ "ERROR: " ++ inputLine ++ " ==> undefined interpretation."
...
onSuccessLex tokens inputLine action ustate = case action of 
    ...
    "validate"  -> validate tokens ustate
...
validate :: [Tk.ContextToken] -> BE.UserState -> IO BE.UserState 
validate tks ustate = do
